...
- Une période = des dates et le ou les prix écrits pour elles. Des dates sans aucun prix (« Disponibilités : du 18 au 25 juillet ») ne sont pas une période. Une liste de mois disponibles (« Disponible : avril, octobre ») non plus : ne lui colle jamais les prix écrits sur d'autres lignes.
...
- Dates au format AAAA-MM-JJ. Une date écrite est le jour du DÉPART, donc la fin : « du 12 au 19 juin » → start 06-12, end 06-19. Un mois entier va jusqu'au 1er du mois suivant : « mai et juin » → 05-01 à 07-01 ; « fin août » et « au 31 août » → 09-01.
- Année : celle écrite, sinon celle de la date de publication donnée avec l'annonce. Une année sur deux chiffres (« 14/07/28 ») = 20xx.
- Dates floues (« basse saison », « hors saison », « mi-juin », « début juillet », « les premières semaines de juin », « le reste du temps ») : garde le prix, mets `null` aux dates inconnues. N'invente jamais une date de début ou de fin. Une date connue reste remplie (« début mai → 20 juin » → start null, end 06-20).
...
Seulement des dates écrites comme prises (« réservé », « loué », « complet », « pas de dispo »), avec deux bornes (« réservé du 3 au 17 juillet », une saison dite complète si ses mois sont écrits, des mois exclus de la location : « location hors juillet/août » → juillet et août indisponibles). « Libre à partir du 10 août » ou « pas de dispo jusqu'au 10 août » ne disent pas depuis quand c'est pris : rien. Une annonce qui liste ses semaines une par une avec « déjà loué » : une indisponibilité par semaine louée, n'en oublie aucune. Fin = jour de départ, comme les tarifs.
...
- `surface` : surface habitable seulement ; la surface d'une terrasse, d'une parcelle ou d'un emplacement n'en est pas une (« parcelle de 100 m2 » → null).
...
- `otherFeatures` : les équipements ou atouts écrits qui ne sont PAS dans la liste (balançoire, parasol, lave-mains, plaque vitrocéramique…), un élément par entrée, de 1 à 4 mots, jamais une phrase. Rien n'est jeté, mais ce qui est dans la liste va dans `amenities`, pas ici.
